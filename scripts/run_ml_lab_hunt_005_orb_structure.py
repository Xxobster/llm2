"""ML lab hunt 005 — ORB / structure / NR7 / ADX (not a hunt-004 retune).
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_hunt_005_orb_structure.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_hunt_005_orb_structure"
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "hunt_005_orb_structure_latest.json").write_text(
    ...
    (OUT / "hunt_005_orb_structure_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    append_ledger(f"ML_LAB_HUNT_005 start {stamp} arms={len(plan)}", tier=0)
    ...
    print(
        f"WROTE hunt_005_orb_structure_latest.md skill={payload['n_skill_pass']} "
