"""Pivot LIMIT improve gen 002 — add BTC + deepen placement winners.

RESEARCH_ONLY. Arms frozen before PF. Rank by intent expectancy.
Skip adverse cancel (rejected in 001). Carry ETH q50 / SOL atr_clip / SOL w5.
"""
...
# symbol -> default level_mode for control
SYMBOLS = (
    ("BTCUSDT", "ret", "BTC"),
    ("ETHUSDT", "atr", "ETH"),
    ("SOLUSDT", "ret", "SOL"),
...
def _leak(symbol: str) -> dict:
    ...
    try:
        ...
        return {"symbol": symbol, "status": "PASS"}
    except Exception as exc:  # noqa: BLE001
        return {"symbol": symbol, "status": "FAIL", "error": str(exc)[:300]}
...
def _as_float(v, default=float("nan")) -> float:
    ...
    if isinstance(v, (int, float, np.floating, np.integer)):
        return float(v)
    for attr in ("daily_raw", "raw", "annualized", "value"):
        if hasattr(v, attr):
            try:
...
def _bt(symbol, ohlcv, signals, *, tag: str, max_hold: int):
    ...
    sharpe = _as_float(getattr(m, "sharpe_daily_raw", None))
    if not np.isfinite(sharpe):
        sharpe = _as_float(getattr(m, "sharpe", None))
    return {
        "status": "RAN",
...
def _atr_clip_level(sc, lo: float = 0.8, hi: float = 1.5) -> np.ndarray:
    ...
    mag = np.abs(lr)
    out = np.sign(lr) * np.clip(mag, lo * atr, hi * atr)
    out[~np.isfinite(lr) | ~np.isfinite(atr)] = np.nan
    return out


def _pull_level(sc, frac: float = 0.85) -> np.ndarray:
    """Pull predicted distance toward market (smaller |level| -> more fills)."""
    lr = np.asarray(sc.level_ret, dtype=float).copy()
    out = lr * float(frac)
    out[~np.isfinite(lr)] = np.nan
    return out


def _run(symbol, sc, mask, *, tag: str, work: int, level_override=None):
...
def _promote(ctrl: dict, arm: dict, *, ece_ok: bool = True) -> dict:
    if arm.get("status") != "RAN" or ctrl.get("status") != "RAN":
    ...
    pf_ok = float(arm.get("profit_factor") or 0) >= 0.85 * float(ctrl.get("profit_factor") or 1)
    pnl_ok = float(arm.get("net_pnl") or -1e9) >= float(ctrl.get("net_pnl") or 0) * 0.95
    # volume path: prefer more trades OR higher pnl with intent-exp up
    return {
        ...
        "promote_volume": bool(exp_ok and pf_ok and ece_ok and pnl_ok),
    }
...
def main() -> int:
    ...
    print(f"exec improve 002 stamp={stamp}", flush=True)

    leak = [_leak(s) for s, _, _ in SYMBOLS]
    for x in leak:
    ...
    frozen = {
        ...
        "ETHUSDT": [
            "ETH_ctrl_atr_w4",
            "ETH_level_q50_w4",
            "ETH_level_q50_pull85_w4",
            "ETH_mid_atr_band_w4",
        ],
        "SOLUSDT": [
            "SOL_ctrl_ret_w4",
            "SOL_level_atr_clip_w4",
            "SOL_level_atr_clip_w5",
            "SOL_level_q50_w4",
            "SOL_mid_atr_band_w4",
        ],
    }
    freeze_path = out_dir / f"exec_improve_002_freeze_{stamp}.json"
    freeze_path.write_text(
        json.dumps(
            {
                "stamp": stamp,
                "preregister": "configs/preregister/pivot_exec_improve_002.yaml",
                "frozen_arms_before_pf": frozen,
    ...
    report = {
        "generation_id": "pivot_exec_improve_002",
        "stamp": stamp,
    ...
    for sym, level_mode, prefix in SYMBOLS:
        ...
        sc_atr = None
        if sym == "BTCUSDT":
            print(f"SCORE {sym} level=atr …", flush=True)
            sc_atr = score_symbol_oos(
                sym,
                timeframe=TF,
                horizon_bars=H,
                feature_pack=PACK,
        ...
        clip = _atr_clip_level(sc)
        pull = _pull_level(sc, 0.85)
        q_mask = gate_mask(sc_q, mode="p75", tp=TP, sl=SL)
        q_pull = _pull_level(sc_q, 0.85)

        arms: list[dict] = []
        if sym == "BTCUSDT":
            arms.append(_run(sym, sc, base, tag="BTC_ctrl_ret_w4", work=4))
            assert sc_atr is not None
            arms.append(
                _run(
                    sym,
                    sc_atr,
                    gate_mask(sc_atr, mode="p75", tp=TP, sl=SL),
                    tag="BTC_ctrl_atr_w4",
                    work=4,
                )
            )
            arms.append(_run(sym, sc_q, q_mask, tag="BTC_level_q50_w4", work=4))
            arms.append(
                _run(sym, sc, base, tag="BTC_level_atr_clip_w4", work=4, level_override=clip)
            )
            arms.append(_run(sym, sc, mid, tag="BTC_mid_atr_band_w4", work=4))
            arms.append(_run(sym, sc, base, tag="BTC_pull85_w4", work=4, level_override=pull))
            arms.append(_run(sym, sc, base, tag="BTC_sibling_w5", work=5))
            ctrl = next(a for a in arms if a["tag"] == "BTC_ctrl_ret_w4")
        elif sym == "ETHUSDT":
            arms.append(_run(sym, sc, base, tag="ETH_ctrl_atr_w4", work=4))
            arms.append(_run(sym, sc_q, q_mask, tag="ETH_level_q50_w4", work=4))
            arms.append(
                _run(
                    sym,
                    sc_q,
                    q_mask,
                    tag="ETH_level_q50_pull85_w4",
                    work=4,
                    level_override=q_pull,
                )
            )
            arms.append(_run(sym, sc, mid, tag="ETH_mid_atr_band_w4", work=4))
            ctrl = next(a for a in arms if a["tag"] == "ETH_ctrl_atr_w4")
        else:
            arms.append(_run(sym, sc, base, tag="SOL_ctrl_ret_w4", work=4))
            arms.append(
                _run(sym, sc, base, tag="SOL_level_atr_clip_w4", work=4, level_override=clip)
            )
            arms.append(
                _run(sym, sc, base, tag="SOL_level_atr_clip_w5", work=5, level_override=clip)
            )
            arms.append(_run(sym, sc_q, q_mask, tag="SOL_level_q50_w4", work=4))
            arms.append(_run(sym, sc, mid, tag="SOL_mid_atr_band_w4", work=4))
            ctrl = next(a for a in arms if a["tag"] == "SOL_ctrl_ret_w4")
        ...
        for a in arms:
            ...
            promotions[a["tag"]] = _promote(ctrl, a, ece_ok=ece_ok)

        ranking = _rank(arms)
        # recommended: among promote_volume True, else best promote, else ctrl
        vol = [
            a
            for a in arms
            if promotions.get(a["tag"], {}).get("promote_volume")
        ]
        if vol:
            pick = sorted(
                vol,
            ...
            prom = [a for a in arms if promotions.get(a["tag"], {}).get("promote")]
            pick = (
                sorted(
                    prom,
                    key=lambda a: -float(a.get("expectancy_intent_all") or -1e9),
                )[0]
                if prom
                else ctrl
            )
        best_per_sym[sym] = pick["tag"]
        ...
        print(f"  RANK top3 {sym}: {ranking[:3]}", flush=True)
        print(f"  PICK {sym}: {pick['tag']}", flush=True)

    report["recommended_trio"] = best_per_sym
    path = out_dir / f"exec_improve_002_{stamp}.json"
    latest = out_dir / "exec_improve_002_latest.json"
    path.write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
    latest.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    md = out_dir / f"exec_improve_002_{stamp}.md"
    md.write_text(_md(report), encoding="utf-8")
    print(f"WROTE {path}", flush=True)
    print(f"RECOMMENDED TRIO: {best_per_sym}", flush=True)
    try:
...
def _md(r: dict) -> str:
    lines = [
        f"# Exec improve 002 (`{r['stamp']}`)",
        "",
        f"**{r['readiness_max']}** — BTC+ETH+SOL; rank by intent expectancy",
        "",
        f"**Recommended trio:** `{r.get('recommended_trio')}`",
        "",
    ]
    for sym, block in r["symbols"].items():
        d = block["diagnostics"]
        lines.append(f"## {sym} (pick=`{block['pick']}`)")
        lines.append(
        ...
        lines.append("")
        for k, v in block["promotions_vs_ctrl"].items():
            lines.append(
                f"- `{k}`: promote={v.get('promote')} volume={v.get('promote_volume')} `{v}`"
            )
        lines.append("")
    return "\n".join(lines)
