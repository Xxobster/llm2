"""Nested settle: ETH K5 double + isotonic hit-cal + π* vs live control.

Outer folds only. Primary metric: expectancy_return_units.
"""
...
from llm2.decision.calibrate import (  # noqa: E402
...
GEN = "structure_v1_eth_k5_ev_calibrated_pi_star_001"
PREREG = _ROOT / "configs" / "preregister" / f"{GEN}.yaml"
CONTROL_PACK = ARTIFACTS / "live_packs" / "structure_v1_ethusdt_k5_double3h_v1"
ARM = SingleBookArm(clarity="mean_strength", horizon_bars=12, tp_pct=TP, sl_pct=SL)
HURDLE = CostHurdle(tp_pct=TP, sl_pct=SL)
PI_STAR = float(HURDLE.break_even_probability())
CALIB_FRAC = 0.20  # last 20% of each fold train for isotonic
...
def _apply_pi_star_mask(
    ...
    *,
    ...
    """Zero side where calibrated hit prob < π* (candidate) or leave raw (control)."""
    s = np.asarray(side, dtype=int).copy()
    stats = {"n_before": int(np.sum(s != 0)), "n_skipped_pi_star": 0, "n_after": 0}
    if not use_cal:
        stats["n_after"] = stats["n_before"]
        return s, stats
    abs_m = np.abs(np.asarray(mean, dtype=float))
    p_hat = cal.predict_proba_hit(abs_m)
    drop = (s != 0) & (p_hat < PI_STAR)
    stats["n_skipped_pi_star"] = int(np.sum(drop))
    s[drop] = 0
    stats["n_after"] = int(np.sum(s != 0))
    return s, stats


def stitch_arm(*, use_calibration: bool, label: str) -> dict[str, Any]:
    ...
    all_ru: list[float] = []
    n_liq = 0
    calib_notes: list[dict[str, Any]] = []
    pi_stats_total = {"n_before": 0, "n_skipped_pi_star": 0, "n_after": 0}
    emit_stats = {
        "n_emitted": 0,
        "n_skipped_clarity": 0,
        "n_skipped_cap": 0,
    ...
    high_full = ohlcv["high"].reindex(aligned.index).to_numpy(dtype=float)
    low_full = ohlcv["low"].reindex(aligned.index).to_numpy(dtype=float)

    for fold in folds:
        tr, oos = fold.train_indices, fold.oos_indices
        n_tr = len(tr)
        split, n_cal = train_calib_split(
            n_tr, calib_frac=CALIB_FRAC, min_calib=200, min_fit=500
        )
        # Control (and baseline) fit on full train; candidate holds out last calib_frac.
        if use_calibration:
            tr_fit = tr[:split]
            tr_cal = tr[split:]
        else:
            tr_fit = tr
            tr_cal = tr[split:]  # unused
            n_cal = 0
        model = LGBMRegressorPredictor(n_estimators=80, learning_rate=0.05)
        model.fit(X[tr_fit], yv[tr_fit])

        cal = None
        if use_calibration and n_cal >= 80:
            pred_cal = model.predict(X[tr_cal])
            mean_cal = np.asarray(pred_cal.mean, dtype=float).reshape(-1)
            # Bracket path win labels align π* to TP/SL geometry (not raw direction).
            abs_m, hit, over = bracket_path_hit_labels(
                ...
                tp_pct=TP,
                sl_pct=SL,
                hold_bars=int(ARM.horizon_bars),
                min_edge=MIN_EDGE,
            )
            cal = fit_hit_calibrator(abs_m[over], hit[over], min_points=80)
            mean_p = (
                float(np.mean(hit[over])) if np.any(over) else float("nan")
            )
            calib_notes.append(
                {
                    ...
                    "n_fit": int(cal.n_fit),
                    "n_calib_rows": int(n_cal),
                    "n_over_edge": int(np.sum(over)),
                    "hit_rate_calib": mean_p,
                    "ece": cal.ece,
                    "p_hat_median_calib": (
                        float(np.median(cal.predict_proba_hit(abs_m[over])))
                        if cal.fitted and np.any(over)
                        else None
                    ),
                }
            )
        else:
            from llm2.decision.calibrate import HitCalibrator

            cal = HitCalibrator(model=None, fitted=False, n_fit=0)
            calib_notes.append(
                {
                    "fold": int(fold.fold_index),
                    "fitted": False,
        ...
        side, pi_st = _apply_pi_star_mask(
            mean, side, cal=cal, use_cal=bool(use_calibration and cal.fitted)
        )
        for k in pi_stats_total:
            pi_stats_total[k] += int(pi_st.get(k, 0) or 0)
        ...
        for k in emit_stats:
            emit_stats[k] += int(stats.get(k, 0) or 0)
        ...
        print(
            ...
            f"pi_skip={pi_st.get('n_skipped_pi_star')} cal={calib_notes[-1].get('fitted')}",
            flush=True,
        )
    ...
    ru = np.asarray(all_ru, dtype=float)
    ...
    stitched = {
        ...
        "expectancy_return_units": (exp_num / exp_den)
        if exp_den
    ...
    return {
        ...
        "pi_star": PI_STAR,
        "hurdle": {
            "mu_plus": HURDLE.mu_plus,
            "mu_minus": HURDLE.mu_minus,
            "pi_star": PI_STAR,
        },
        "folds": fold_rows,
        "stitched": stitched,
        "pi_stats_total": pi_stats_total,
        "signal_stats_total": emit_stats,
...
def main() -> int:
    ...
    print(f"[{GEN}] pi_star={PI_STAR:.4f} mu+={HURDLE.mu_plus:.5f} mu-={HURDLE.mu_minus:.5f}")
    print(f"[{GEN}] control …", flush=True)
    control = stitch_arm(use_calibration=False, label=f"{GEN}_control")
    print(
        f"  control exp_ru={control['stitched']['expectancy_return_units']} "
        f"pf={control['stitched']['profit_factor']} n={control['stitched']['n_trades']}",
        flush=True,
    )
    print(f"[{GEN}] candidate isotonic+pi* …", flush=True)
    candidate = stitch_arm(use_calibration=True, label=f"{GEN}_candidate")
    print(
        f"  candidate exp_ru={candidate['stitched']['expectancy_return_units']} "
        f"pf={candidate['stitched']['profit_factor']} n={candidate['stitched']['n_trades']} "
        f"pi_skips={candidate['pi_stats_total']}",
        flush=True,
    )
    ...
    passed = bool(beat and pf_ok and no_liq)
