...
RESEARCH_ONLY. Evaluates the 12 frozen hunt 001 survivors once per outer fold and
once on the stitched outer-OOS union. Never re-searches a knob, never deploys.
Resumable via SQLite checkpoint.

Usage:

    python -u scripts/run_edge_lab_nested_settle_001.py
    python -u scripts/run_edge_lab_nested_settle_001.py --report-only
"""
...
PREREG = _ROOT / "configs" / "preregister" / "edge_lab_nested_settle_001.yaml"
OUT = ARTIFACTS / "reports" / "edge_lab"
DB_DIR = ARTIFACTS / "sqlite" / "edge_lab_nested_settle_001"
...
def _run(ctx: Ctx, spec: dict, window: np.ndarray, tag: str, cfg: dict) -> dict[str, Any]:
    ...
    if _is_market(event):
        return run_market_arm(
            spec["symbol"], ctx.sc, mask, is_short, tag=tag, max_hold=max_hold, tp=tp, sl=sl
        )
    lim = _limit_atr(ctx.sc, mask, is_short)
    return run_limit_arm(
        spec["symbol"], ctx.sc, mask, is_short, lim,
        tag=tag, work=int(cfg["work_bars"][tf]), max_hold=max_hold, tp=tp, sl=sl,
    )
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    payload = {
        "generation_id": "edge_lab_nested_settle_001",
        "stamp": stamp,
    ...
    md = [
        ...
        "pre-2022 inner screen before these bars were scored.",
        "",
        ...
        "positions are minimum-exchange size against the research wallet, so exposure "
        "runs a few percent and drawdown lands near 0.02%. Profit factor, win rate, "
        "payoff, Sharpe, HAC Sharpe, Sortino and entry-bar rate are scale-free.",
        "",
        "| Symbol | TF | Event | Session | TP | SL | n | /mo | PF | WR | Payoff | Sharpe | "
        "HAC | Sortino | ebr | fill% | folds+ | gate |",
        "|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for r in stitched:
        ...
        md.append(
            ...
            "{po} | {sh} | {hac} | {so} | {ebr} | {fp} | {fo} | {v} |".format(
                sym=r.get("symbol"), tf=r.get("timeframe"), ev=r.get("event"),
                ses=r.get("session_filter"), tp=r.get("tp"), sl=r.get("sl"),
                ...
                fo=f"{r.get('n_folds_positive')}/{r.get('n_folds_ran')}", v=verdict,
            )
        )
...
def main() -> int:
    ...
    append_ledger(f"EDGE_LAB_NESTED_SETTLE_001 start {stamp} arms={len(survivors)}", tier=0)
    ...
    payload = write_report(con, cfg, stamp, extra)
    append_ledger(
        f"EDGE_LAB_NESTED_SETTLE_001 done stitched={payload['n_stitched']} "
        f"pass={payload['n_gate_pass']}",
        tier=0,
    )
    print(
        f"WROTE {OUT / 'nested_settle_001_latest.md'} stitched={payload['n_stitched']} "
