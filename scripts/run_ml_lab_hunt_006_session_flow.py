"""ML lab hunt 006 — session / flow (not a hunt-004/005 retune).
...
from llm2.ml_lab.idea_catalog_c import (  # noqa: E402
    IDEA_IDS_C,
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_hunt_006_session_flow.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_hunt_006_session_flow"
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "hunt_006_session_flow_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    plan = [
        ...
        for idea in IDEA_IDS_C
    ]
    append_ledger(f"ML_LAB_HUNT_006 start {stamp} arms={len(plan)}", tier=0)
    ...
    for spec in plan:
        ...
        if ctx_key != key:
            ...
            if len(ohlcv) < MIN_CONTEXT_BARS:
                ...
                ctx_key = key
                continue
            sigs = signals_c(ohlcv, spec["timeframe"])
    ...
    print(
        f"WROTE hunt_006_session_flow_latest.md skill={payload['n_skill_pass']} "
