"""ML lab hunt 010 — contaminated tunes of 004 / 007 / 008.

RESEARCH_ONLY_CONTAMINATED. Pre-lockbox window (nested OOS allowed). Never deploys.
...
from llm2.ml_lab.idea_catalog_g import (  # noqa: E402
    IDEA_IDS_G,
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_hunt_010_contaminated_tune.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_hunt_010_contaminated_tune"
COSTS = research_maker_first_costs()


def _tune_end_ms() -> int:
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    payload = {
        ...
        "stamp": stamp,
        "readiness_max": "RESEARCH_ONLY_CONTAMINATED",
        "n_rows": len(rows),
    ...
    (OUT / "hunt_010_contaminated_tune_latest.json").write_text(
        json.dumps(jsonable(payload), indent=2), encoding="utf-8"
    )
    md = [
        "# ML lab hunt 010 (contaminated tunes of 004 / 007 / 008)",
        "",
        f"**Max readiness:** `RESEARCH_ONLY_CONTAMINATED`. Stamp `{stamp}`.",
        f"Rows: {len(rows)}. Skill pass: **{n_skill}**. Screen pass: **{n_pass}**.",
        "Maker 0.02% resting legs. EXEC-021. Pre-lockbox window (nested OOS viewed — not Shadow-Ready).",
        "",
    ...
    (OUT / "hunt_010_contaminated_tune_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    plan = [
        ...
        for idea in IDEA_IDS_G
    ]
    append_ledger(f"ML_LAB_HUNT_010 start {stamp} arms={len(plan)} CONTAMINATED", tier=0)
    ...
    for spec in plan:
        ...
        if ctx_key != key:
            ...
            try:
                ...
                ohlcv = raw.loc[index_to_ms(raw.index) < _tune_end_ms()].copy()
            ...
            if len(ohlcv) < MIN_CONTEXT_BARS:
                ...
                ctx_key = key
                continue
            sigs = signals_g(ohlcv, spec["timeframe"])
    ...
    print(
        f"WROTE hunt_010_contaminated_tune_latest.md skill={payload['n_skill_pass']} "
