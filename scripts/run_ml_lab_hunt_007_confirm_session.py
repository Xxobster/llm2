"""ML lab hunt 007 — confirm / NY cash / HTF close-location (not a 004–006 retune).
...
from llm2.ml_lab.idea_catalog_d import (  # noqa: E402
    IDEA_IDS_D,
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_hunt_007_confirm_session.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_hunt_007_confirm_session"
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "hunt_007_confirm_session_latest.json").write_text(
        json.dumps(jsonable(payload), indent=2), encoding="utf-8"
    )
    md = [
        "# ML lab hunt 007 (confirm / NY cash / HTF close-location)",
        "",
    ...
    (OUT / "hunt_007_confirm_session_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    plan = [
        ...
        for idea in IDEA_IDS_D
    ]
    append_ledger(f"ML_LAB_HUNT_007 start {stamp} arms={len(plan)}", tier=0)
    ...
    for spec in plan:
        ...
        if ctx_key != key:
            ...
            if len(ohlcv) < MIN_CONTEXT_BARS:
                ...
                ctx_key = key
                continue
            sigs = signals_d(ohlcv, spec["timeframe"])
    ...
    print(
        f"WROTE hunt_007_confirm_session_latest.md skill={payload['n_skill_pass']} "
