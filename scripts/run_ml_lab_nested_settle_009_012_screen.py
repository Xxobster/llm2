"""Nested outer settle of hunt-009 / hunt-012 screen passers (+ 011 diagnostic).
...
from llm2.ml_lab.idea_catalog_f import signals_f  # noqa: E402
from llm2.ml_lab.idea_catalog_h import signals_h  # noqa: E402
from llm2.ml_lab.idea_catalog_i import signals_i  # noqa: E402
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_nested_settle_009_012_screen.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_nested_settle_009_012_screen"
COSTS = research_maker_first_costs()
CATALOGS = {"f": signals_f, "h": signals_h, "i": signals_i}
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "nested_settle_009_012_screen_latest.json").write_text(
    ...
    (OUT / "nested_settle_009_012_screen_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    append_ledger(f"ML_LAB_NESTED_SETTLE_009_012 start {stamp} arms={len(survivors)}", tier=0)
    ...
    append_ledger(
        f"ML_LAB_NESTED_SETTLE_009_012 done stitched={payload['n_stitched']} pass={payload['n_gate_pass']}",
        tier=0,
    )
    print(f"WROTE {OUT / 'nested_settle_009_012_screen_latest.md'}", flush=True)
    return 0
