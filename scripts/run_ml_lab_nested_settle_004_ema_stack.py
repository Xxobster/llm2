"""ML lab nested outer settle 004 — two frozen EMA-stack names.
...
from llm2.gates.evidence import research_costs_moderate_stress  # noqa: E402
from llm2.ml_lab.idea_catalog import signals_from_ohlcv  # noqa: E402
from llm2.ml_lab.v21_battery import HUNT004_EVENT_ROWS, evaluate_nested_v21  # noqa: E402
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_nested_settle_004_ema_stack.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_nested_settle_004_ema_stack"
...
class Ctx:
    def __init__(self, symbol: str, tf: str) -> None:
        ...
        o2 = o2.loc[index_to_ms(o2.index) < _lock_ms()]
        aligned = o2["close"].reindex(ohlcv.index).ffill()
        self.sig = signals_from_ohlcv(ohlcv, tf, other_close=aligned.to_numpy(dtype=float))
...
def _run(ctx: Ctx, spec: dict, window: np.ndarray, tag: str, cfg: dict, *, costs=None, market: bool = False) -> dict[str, Any]:
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "nested_settle_004_ema_stack_latest.json").write_text(
        json.dumps(jsonable(payload), indent=2), encoding="utf-8"
    )
    md = [
        "# ML lab nested settle 004 (EMA stack)",
        "",
        f"**Max readiness:** `LIVE_STOP / RESEARCH_ONLY`. Stamp `{stamp}`.",
        f"Stitched arms: {len(stitched)}. Gate pass: **{payload['n_gate_pass']}**.",
        "Window: 2022-01-01 to lockbox 2026-05-01. No retune.",
        "",
        "| Symbol | TF | n | PF | Sharpe | HAC | ebr | folds+ | gate |",
        "|---|---|---:|---:|---:|---:|---:|---|---|",
    ]
    for r in stitched:
        md.append(
            "| {s} | {tf} | {n} | {pf} | {sh} | {hac} | {ebr} | {p}/{ran} | {g} |".format(
                s=r.get("symbol"),
                tf=r.get("timeframe"),
                n=r.get("n_trades"),
    ...
    (OUT / "nested_settle_004_ema_stack_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    append_ledger(f"ML_LAB_NESTED_SETTLE_004 start {stamp} arms={len(survivors)}", tier=0)
    ...
    for spec in survivors:
        ...
        for f in ctx.folds:
            ...
            print(
                f"  fold{f.fold_index} {aid} -> {payload.get('status')} n={payload.get('n_trades')} "
        ...
        if not arm_done(con, aid):
            ...
            print(
                f"  STITCHED {aid} -> {payload.get('status')} n={payload.get('n_trades')} "
                f"PF={_fmt(payload.get('profit_factor'))} folds+={pos}/{ran} "
                f"gate={'PASS' if ok else ','.join(fails)}",
                flush=True,
            )
            write_report(con, cfg, stamp, extra)

        vid = _arm_id(spec, "v21")
        if not arm_done(con, vid):
            fold_rows = [
                r
                for r in load_arms(con)
            ...
            print(f"=== V2.1 remainder {spec} ===", flush=True)
            try:
                baseline = _run(ctx, spec, ctx.oos_union, vid + "|base", cfg)
                stress = _run(
                    ctx,
                    spec,
                    ctx.oos_union,
                    vid + "|stress",
                    cfg,
                    costs=research_costs_moderate_stress(),
                    market=True,
                )
                ...
                v21_payload = {
                    **spec,
                    "kind": "v21",
                    "status": "RAN",
                    "n_trades": int(baseline.get("n_trades") or 0),
                    "overall": gates.get("overall"),
                    "gates": {k: v for k, v in gates.items() if k != "dsr_sensitivity"},
                    "dsr_sensitivity": gates.get("dsr_sensitivity"),
                    "baseline": {k: baseline.get(k) for k in (
                        "n_trades", "profit_factor", "trades_per_month", "sharpe_annualised",
                        "sharpe_hac_annualised", "entry_bar_exit_rate", "fill_pct",
                        "net_pnl", "win_rate", "n_liquidations", "peak_margin_util",
                        "n_daily_returns",
                    )},
                    "stress": {k: stress.get(k) for k in (
                        "n_trades", "profit_factor", "trades_per_month", "net_pnl",
                        "n_liquidations", "entry_bar_exit_rate",
                    )},
                }
            except Exception as exc:  # noqa: BLE001
                v21_payload = {**spec, "kind": "v21", "status": "ERROR", "error": str(exc)[:400], "n_trades": 0}
            save_arm(con, vid, spec, v21_payload)
            print(
                f"  V21 {vid} -> {v21_payload.get('status')} overall={v21_payload.get('overall')}",
                flush=True,
            )
            write_report(con, cfg, stamp, extra)
    ...
    payload = write_report(con, cfg, stamp, extra)
    append_ledger(
        f"ML_LAB_NESTED_SETTLE_004 done stitched={payload['n_stitched']} pass={payload['n_gate_pass']}",
        tier=0,
    )
    print(
        f"WROTE {OUT / 'nested_settle_004_ema_stack_latest.md'} gate_pass={payload['n_gate_pass']}",
        flush=True,
    )
    return 0
