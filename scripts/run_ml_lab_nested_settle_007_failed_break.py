"""Nested outer settle of hunt-007 Bitcoin 1h failed_break_fade.
...
from llm2.ml_lab.idea_catalog_d import signals_d  # noqa: E402
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_nested_settle_007_failed_break.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_nested_settle_007_failed_break"
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "nested_settle_007_failed_break_latest.json").write_text(
        json.dumps(jsonable(payload), indent=2), encoding="utf-8"
    )
    md = [
        "# ML lab nested settle 007 (failed-break fade)",
        "",
        ...
        "",
        "| Symbol | TF | n | /mo | PF | Sharpe | HAC | ebr | folds+ | gate |",
        "|---|---|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for r in stitched:
        md.append(
            "| {s} | {tf} | {n} | {tpm} | {pf} | {sh} | {hac} | {ebr} | {p}/{ran} | {g} |".format(
                s=r.get("symbol"),
                tf=r.get("timeframe"),
                n=r.get("n_trades"),
    ...
    (OUT / "nested_settle_007_failed_break_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    append_ledger(f"ML_LAB_NESTED_SETTLE_007 start {stamp} arms={len(survivors)}", tier=0)
    print(f"preregister_sha256={prereg_sha} survivors={len(survivors)}", flush=True)
    spec = survivors[0]
    print(f"=== context {spec['symbol']} {spec['timeframe']} ===", flush=True)
    ...
    append_ledger(
        f"ML_LAB_NESTED_SETTLE_007 done stitched={payload['n_stitched']} pass={payload['n_gate_pass']}",
        tier=0,
    )
    print(f"WROTE {OUT / 'nested_settle_007_failed_break_latest.md'}", flush=True)
    return 0
