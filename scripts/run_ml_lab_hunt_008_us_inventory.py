"""ML lab hunt 008 — US-hours inventory / weekend range (not a 004–007 retune).
...
from llm2.ml_lab.idea_catalog_e import (  # noqa: E402
    IDEA_IDS_E,
...
PREREG = _ROOT / "configs" / "preregister" / "ml_lab_hunt_008_us_inventory.yaml"
OUT = ARTIFACTS / "reports" / "ml_lab"
DB_DIR = ARTIFACTS / "sqlite" / "ml_lab_hunt_008_us_inventory"
...
def write_report(con, cfg: dict, stamp: str, extra: dict) -> dict[str, Any]:
    ...
    (OUT / "hunt_008_us_inventory_latest.json").write_text(
        json.dumps(jsonable(payload), indent=2), encoding="utf-8"
    )
    md = [
        "# ML lab hunt 008 (US-hours inventory / weekend range)",
        "",
    ...
    (OUT / "hunt_008_us_inventory_latest.md").write_text("\n".join(md) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    plan = [
        ...
        for idea in IDEA_IDS_E
    ]
    append_ledger(f"ML_LAB_HUNT_008 start {stamp} arms={len(plan)}", tier=0)
    ...
    for spec in plan:
        ...
        if ctx_key != key:
            ...
            if len(ohlcv) < MIN_CONTEXT_BARS:
                ...
                ctx_key = key
                continue
            sigs = signals_e(ohlcv, spec["timeframe"])
    ...
    print(
        f"WROTE hunt_008_us_inventory_latest.md skill={payload['n_skill_pass']} "
