"""Settle ETH 15m multitrade geometry arms (wall-clock / bar-count × p75).
...
AUDIT = ARTIFACTS / "reports" / "structure_v1_eth_15m_warehouse_audit_latest.json"
TIMEFRAME = "15m"
SYMBOL = "ETHUSDT"
...
def main() -> int:
    ...
    if not AUDIT.is_file():
        raise SystemExit(
            ...
            "scripts/build_eth_15m_structure_and_audit.py first"
        )
    ...
    # Rank by primary metric among arms with finite E[r] and n>0
    ranked = []
    ...
    report = stamp_min_size_equity_caveat(
        {
            ...
            "note": (
                "All three arms preregistered before OOS. MIN_EXCHANGE dust caveat. "
                "Do not reopen 1h clarity×hold×TP. BTC Phase 2 needs new prereg."
            ),
        }
    )
    ...
    register_compare_report(
        ...
        tradesim_run_ids=[],
        control_version_id="eth_15m_wall_clock_q50",
    )
